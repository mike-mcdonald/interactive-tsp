<template>
  <section class="relative">
    <div ref="chart" class="flex flex-row">
      <router-link
    ...
    <div
      v-for="entry in dataset"
      :key="entry.classificationValue"
      v-show="show[entry.classificationValue]"
      class="absolute z-50 inset-x-0 top-0 mt-6 px-2 py-1 bg-white border border-black rounded"
    >{{ entry.label }}: {{ entry.count }} / {{ total }}</div>
  </section>
</template>
...
import { ClassificationAnalysisData } from '../store/streets/types';
